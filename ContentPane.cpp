...
auto ContentPane::IntoNewPath(QString newPath, bool isNewPath, bool isF5Force) -> bool{
...
void ContentPane::subscribe() {
    connect(view, &QTableView::doubleClicked, this, &ContentPane::on_cellDoubleClicked);
    connect(view->selectionModel(), &QItemSelectionModel::selectionChanged, this, &ContentPane::on_selectionChanged);
}
...
auto ContentPane::on_selectionChanged(const QItemSelection &selected, const QItemSelection &deselected)->bool{
    if (selected.isEmpty()){
        return true;
    }
    ...
    anchorTags.insert(pth, {firstIndex.row(), firstIndex.column()});

    qDebug("\t\t Anchor of path [%s] target to [%d,%d]", pth.toStdString().c_str(), anchorTags[pth].row, anchorTags[pth].col);
//    previewHtml->operator()(firstFileInfo.absoluteFilePath());
//    previewWidget->operator()(firstFileInfo.absoluteFilePath());
