...
pause)"
  ...
  const bool writeResult = FileTool::StringTextWriter(batNativeFileAbsPath, addContextMenuBatContent, QIODevice::WriteOnly | QIODevice::Text);
  if (!writeResult) {
...
pause)"
  ...
  const bool writeResult = FileTool::StringTextWriter(batNativeFileAbsPath, rmvContextMenuBatContent, QIODevice::WriteOnly | QIODevice::Text);
  if (!writeResult) {
    LOG_W("[Failed] When write into file %s", qPrintable(batNativeFileAbsPath));
    return false;
  }
