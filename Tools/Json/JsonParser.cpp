...
InsertByJsonParseResult ValidateSampleMd5AndVidName(const int localSampleMD5Size, const int localVidNameSize) {
  ...
  if (localSampleMD5Size == 0 && localVidNameSize == 0) {
    return InsertByJsonParseResult::IGNORE_NO_NEED_FURTHER_PROCESS;
  }
  // 有且仅有1个空 => 原因: 未计算MD5的json, 手动删去了vidName
  if (localSampleMD5Size == 0 || localVidNameSize == 0) {
    return InsertByJsonParseResult::ERROR;
  }
  return InsertByJsonParseResult::OK_NEED_FURTHER_PROCESS;
}
...
InsertByJsonParseResult ParseEssentialFieldJson(const QString& jsonFilePath,
  ...
  if (checkResult != InsertByJsonParseResult::OK_NEED_FURTHER_PROCESS) {
    return checkResult;
  }
