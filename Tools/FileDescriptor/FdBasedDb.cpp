...
const QString FdBasedDb::CREATE_TABLE_TEMPLATE                                                //
    {"CREATE TABLE IF NOT EXISTS `%1` ("                                                      // TABLE_NAME
     + QString{"`%1` CHAR(32) NOT NULL, "}                                                    // SampleMD5
     ...
     + QString{"`%1` INTEGER DEFAULT %2, "                                                    // Size default 0
               "`%3` INTEGER DEFAULT %4, "}                                                   // Duration default 0
           .arg(ENUM_2_STR(Size))                                                             //
           .arg(JsonFieldBoundary::SIZE_INITIAL_VALUE)                                        //
           .arg(ENUM_2_STR(Duration))                                                         //
           .arg(JsonFieldBoundary::DURATION_INITIAL_VALUE)                                    //
...
int FdBasedDb::ReadADirectory(const QString& tableName, const QString& folderAbsPath) {
  ...
  while (it.hasNext()) {
    ...
    if (++fileProcessCnt % JsonFieldBoundary::LOG_PER_CALC_FILE_FD == 0) {
      LOG_W("The %dth of %d files fd have been calculated", fileProcessCnt / JsonFieldBoundary::LOG_PER_CALC_FILE_FD, JsonFieldBoundary::LOG_PER_CALC_FILE_FD);
    }
    if (fdVal.isEmpty()) {
      LOG_E("Read file id failed[%s]", qPrintable(absFilePath));
      continue;
    }
  ...
  int insertCnt = 0;
  FD_ERROR_CODE ret = InsertSimple(tableName, newFd2Pth, insertCnt);
  if (ret != FD_OK) {
    LOG_E("Incremental insert failed errorCode:%d", ret);
    return ret;
  }
...
// Incremental
int FdBasedDb::ReadADirectoryJson(const QString& tableName, const QString& folderAbsPath) {
  if (tableName.isEmpty()) {
    return FD_TABLE_NAME_INVALID;
  }
  ...
  QSqlQuery query{db};
  if (!query.prepare(INSERT_MOVIE_RECORD_FULL_TEMPLATE.arg(tableName))) {
  ...
  JsonOp::ResultE parseRet = JsonOp::ResultE::ERROR;
  ...
  while (it.hasNext()) {
    ...
    if (parseRet == JsonOp::ResultE::ERROR) {
      db.rollback();
      LOG_ERR_NP("Parse Json Failed", jsonAbsPath);
      return FD_JSON_PARSED_INVALID;
    }
    if (parseRet == JsonOp::ResultE::IGNORE_NO_NEED_FURTHER_PROCESS) {
      continue;
    }
  ...
  LOG_O("%d record(s) commit insert into succeed", fileProcessCnt);
  return fileProcessCnt;
}
...
FD_ERROR_CODE FdBasedDb::InsertSimple(const QString& tableName,                    //
  ...
  int count = 0;
  QString absFilePath;
  for (auto it = newFd2Pth.cbegin(); it != newFd2Pth.cend(); ++it) {
    absFilePath = it.value();
    const PathTool::RMFComponent& rmf = PathTool::RMFComponent::FromPath(absFilePath);
    // 绑定参数
    query.bindValue(INSERT_FIELD_Fd, it.key());
...
FD_ERROR_CODE FdBasedDb::Adt(const QString& tableName, const QString& peerPath, VolumeUpdateResult* pAdt) {
  ...
  if (!QueryPK(tableName, ENUM_2_STR(SampleMD5), existedFds)) {
  ...
  const QSet<QByteArray> newFds{newFd2Pth.keyBegin(), newFd2Pth.keyEnd()};
