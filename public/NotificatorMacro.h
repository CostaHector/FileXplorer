...
#define LOG_TITLE_MESSAGE_PATTERN "%s: %s"

// with params
#define LOG_INFO_P(title, msgFormat, ...) \
do { \
      QString _msg = QString::asprintf(msgFormat, ##__VA_ARGS__); \
      LOG_I(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(_msg)); \
      Notificator::information(title, _msg); \
} while(0)
#define LOG_WARN_P(title, msgFormat, ...) \
do { \
      QString _msg = QString::asprintf(msgFormat, ##__VA_ARGS__); \
      LOG_W(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(_msg)); \
      Notificator::warning(title, _msg); \
} while(0)
#define LOG_ERR_P(title, msgFormat, ...) \
do { \
      QString _msg = QString::asprintf(msgFormat, ##__VA_ARGS__); \
      LOG_E(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(_msg)); \
      Notificator::error(title, _msg); \
} while(0)
#define LOG_CRIT_P(title, msgFormat, ...) \
do { \
      QString _msg = QString::asprintf(msgFormat, ##__VA_ARGS__); \
      LOG_C(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(_msg)); \
...
do { \
      QString _msg = QString::asprintf(msgFormat, ##__VA_ARGS__); \
      LOG_D(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(_msg)); \
...
do { \
      LOG_I(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(msg)); \
      Notificator::information(title, msg); \
} while(0)
#define LOG_WARN_NP(title, msg) \
do { \
      LOG_W(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(msg)); \
...
do { \
      LOG_C(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(msg)); \
      Notificator::critical(title, msg); \
} while(0)
#define LOG_OK_NP(title, msg) \
do { \
      LOG_D(LOG_TITLE_MESSAGE_PATTERN, qPrintable(title), qPrintable(msg)); \
