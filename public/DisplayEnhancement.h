...
const QString FILE_SIZE_GMKB_TEMPLATE{"%1'%2'%3'%4"};
const QString FILE_SIZE_DETAIL_TEMPLATE{"%1 (%2 Bytes)"};
inline QString sizeToHumanReadFriendly(const qint64& total) {
...
inline QString sizeToFileSizeDetail(const qint64& total) {
  return FILE_SIZE_DETAIL_TEMPLATE.arg(sizeToHumanReadFriendly(total)).arg(total);
}
inline QString durationToHumanReadFriendly(const qint64& ms) {
