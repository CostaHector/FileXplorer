...
class ReorderableToolBarTest : public PlainTestSuite {
 ...
 private slots:
  void toolButton_no_need_moveWidget() {
    ...
    toolbar.addDraggableAction(nullptr);
  ...
  void toolbutton_DropEvent_ok_EmitSignal_ok() {
    ...
    auto moveInFrontOf = [&](int sourceIndex, int targetIndex) {
      ...
      // will not crash down
      {
        toolbar.dragEnterEvent(nullptr);
        toolbar.dragMoveEvent(nullptr);
        toolbar.dropEvent(nullptr);
      }

      {
        ...
        QDragEnterEvent enterEvent(dropPos, Qt::MoveAction, &mimeData, Qt::LeftButton, Qt::NoModifier);
        toolbar.dragEnterEvent(&enterEvent);
        QVERIFY(!enterEvent.isAccepted());  // mimeData contains not text not accpet

        mimeData.setText("1");
        QVERIFY(mimeData.hasText());
        toolbar.dragEnterEvent(&enterEvent);
        QVERIFY(enterEvent.isAccepted());  // mimeData contains text accpet
        QDragMoveEvent dragEvent(dropPos, Qt::MoveAction, &mimeData, Qt::LeftButton, Qt::NoModifier);
        toolbar.dragMoveEvent(&dragEvent);
        QDropEvent dropEvent(dropPos, Qt::MoveAction, &mimeData, Qt::LeftButton, Qt::NoModifier);
        toolbar.dropEvent(&dropEvent);
