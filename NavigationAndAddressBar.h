...
class NavigationAndAddressBar : public QHBoxLayout {
  Q_OBJECT
public:
  explicit NavigationAndAddressBar(T_IntoNewPath IntoNewPath = nullptr, T_on_searchTextChanged on_searchTextChanged = nullptr,
  ...
  auto InitEventWhenViewChanged() -> void {
    ...
    connect(searchLE, &QLineEdit::textChanged, this, [this]() -> void { m_on_searchTextChanged(searchLE->text()); });
    connect(searchLE, &QLineEdit::returnPressed, this, [this]() -> void { m_on_searchEnterKey(searchLE->text()); });
    connect(winAddress, &AddressELineEdit::intoAPath_active, [this](const QString &pth) -> void { m_IntoNewPath(pth, true, false); });
  }
  ...
  auto onBackspaceEvent() -> bool { return m_IntoNewPath(winAddress->dirname(), true, false); }
