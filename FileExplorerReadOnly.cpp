    ...
    , _statusBar(new CustomStatusBar)
{
    ...
    previewHtmlDock->setAllowedAreas(Qt::DockWidgetArea::LeftDockWidgetArea
...
auto FileExplorerReadOnly::ReadSettings(const QString& initialPath) -> QString
{
    ...
    if (not inputFi.exists()) { // input valid
        openPath = PreferenceSettings()
                       .value("defaultOpenPath", FileExplorerReadOnly::DEFAULT_PATH)
                       .toString();
