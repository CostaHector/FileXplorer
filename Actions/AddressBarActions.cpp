...
AddressBarActions::AddressBarActions(QObject* parent) : QObject{parent} {
  ...
  _LAST_FOLDER = new (std::nothrow) QAction{QIcon(":img/LAST_FOLDER"), "Last folder"};
  ...
  _NEXT_FOLDER = new (std::nothrow) QAction{QIcon(":img/NEXT_FOLDER"), "Next folder"};
...
QActionGroup* AddressBarActions::GetFolderIteratorActions() {
  auto* folderItControlAg = new (std::nothrow) QActionGroup(this);
