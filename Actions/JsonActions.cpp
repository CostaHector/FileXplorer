  ...
  : QObject{parent}                       //
{
  ...
  const QStringList& tags = GetMovieTagsList();
  _ADD_TAGS_ACTIONS_JSON = InitTagsAction(tags);
  _ADD_TAGS_ACTIONS_SCENE = InitTagsAction(tags);
  subcribe();
}

QActionGroup* JsonActions::InitTagsAction(const QStringList& tags) {
  QActionGroup* tagsAG = new QActionGroup{this};
  for (const QString& tag: tags) {
    tagsAG->addAction(new QAction{tag, this});
  }
  return tagsAG;
}
