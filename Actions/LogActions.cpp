...
LogActions::LogActions(QObject* parent) : QObject{parent} {
  _LOG_FILE = new (std::nothrow) QAction{QIcon(":img/FLOW_LOGS"), "Logs"};
  ...
  _LOG_AGING = new (std::nothrow) QAction{"Aging logs"};
  if (_LOG_AGING == nullptr) {
    qCritical("_LOG_AGING is nullptr");
    return;
  }
  _LOG_LEVEL_DEBUG = new (std::nothrow) QAction{"Debug"};
  if (_LOG_LEVEL_DEBUG == nullptr) {
    qCritical("_LOG_LEVEL_DEBUG is nullptr");
    return;
  }
  _LOG_LEVEL_ERROR = new (std::nothrow) QAction{"Error"};
  if (_LOG_LEVEL_ERROR == nullptr) {
    qCritical("_LOG_LEVEL_ERROR is nullptr");
    return;
  }

  _FLUSH_INSTANTLY = new (std::nothrow) QAction{"Force flush"};
  ...
  _LOG_FOLDER->setToolTip(QString("<b>%1 (%2)</b><br/> Show log folder.").arg(_LOG_FOLDER->text(), _LOG_FOLDER->shortcut().toString()));

  _LOG_AGING->setCheckable(false);
  _LOG_AGING->setShortcutVisibleInContextMenu(true);
  _LOG_AGING->setToolTip(QString("<b>%1 (%2)</b><br/> Aging log file is file size >= 20MiB.").arg(_LOG_AGING->text(), _LOG_AGING->shortcut().toString()));
  ...
  _LOG_LEVEL_ERROR->setToolTip(QString("<b>%1 (%2)</b><br/> Set log level to error.").arg(_LOG_LEVEL_ERROR->text(), _LOG_LEVEL_ERROR->shortcut().toString()));
  ...
  _FLUSH_INSTANTLY->setToolTip(QString("<b>%1 (%2)</b><br/> Flush log contents into log file instantly.").arg(_FLUSH_INSTANTLY->text(), _FLUSH_INSTANTLY->shortcut().toString()));
