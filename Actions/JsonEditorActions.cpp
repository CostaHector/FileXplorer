...
JsonEditorActions::JsonEditorActions(QObject* parent)  //
    : QObject{parent}                                  //
{
  _CLR_TO_BE_EDITED_LIST = new (std::nothrow) QAction(QIcon(":img/EMPTY_LISTWIDGET"), "Clear list", this);
  ...
  _DONE_AND_NEXT = new (std::nothrow) QAction(QIcon(""), "Done and next", this);
  _AUTO_SKIP = new (std::nothrow) QAction(QIcon(":img/AUTO_SKIP"), "Autoskip", this);
  _COMPLETE_PERFS_COUNT = new (std::nothrow) QAction(QIcon(), "Skip if", this);
  _QUICK_EDIT_ACTIONS = new (std::nothrow) QActionGroup(this);

  _CAPITALIZE_FIRST_LETTER_OF_EACH_WORD = new (std::nothrow) QAction("Sentense Case", this);
  _LOWER_ALL_WORDS = new (std::nothrow) QAction(QIcon(":img/RENAME_LOWER_CASE"), "lowercase", this);
  ...
  _RENAME_THIS_FILE = new (std::nothrow) QAction(QIcon(":img/RENAME"), "Rename", this);
  _REVEAL_IN_EXPLORER = new (std::nothrow) QAction(QIcon(":img/REVEAL_IN_EXPLORER"), "Reveal", this);
  _OPEN_THIS_FILE = new (std::nothrow) QAction("Open", this);
  _SYSTEM_ACTIONS = GetSystemActions();
  ...
  _SAVE->setToolTip(QString("<b>%1 (%2)</b><br/> Stage current changes")  //
                        .arg(_SAVE->text())                               //
                        .arg(_SAVE->shortcut().toString()));

  _CLR_TO_BE_EDITED_LIST->setToolTip(QString("<b>%1 (%2)</b><br/> Clear to-be-edit list")  //
                                         .arg(_CLR_TO_BE_EDITED_LIST->text())              //
                                         .arg(_CLR_TO_BE_EDITED_LIST->shortcut().toString()));
  ...
  _LOWER_ALL_WORDS->setToolTip(QString("<b>%1 (%2)</b><br/> Lowercase a sentense.")  //
  ...
  _AI_HINT->setToolTip(QString("<b>%1 (%2)</b><br/> Give you performers list hint")  //
                           .arg(_AI_HINT->text())
