  ...
  }//
{
  ...
  const QDir::Filters initialDirFlag{Configuration().value(dirFilterKeyName, BehaviorKey::DIR_FILTER_ON_SWITCH_ENABLE.toVariant()).toInt()};
  // restore Checkable / checked / Enabled;
  setActionCheckedOrNot(initialDirFlag);

  const bool bGrayOrHide{Configuration().value(GetMemoryKeyName(SearchKey::GRAY_ENTRIES_DONT_PASS_FILTER.name), SearchKey::GRAY_ENTRIES_DONT_PASS_FILTER.toVariant()).toBool()};
...
void ModelFilterActions::resetFiltersToInitial() {
  setActionCheckedOrNot(BehaviorKey::DIR_FILTER_ON_SWITCH_ENABLE.v.data.i);
}
