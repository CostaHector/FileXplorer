...
public:
    ...
    auto GetMOVE_COPY_TO_DELETEActions() -> QActionGroup*{
        ...
        MOVE_TO->setToolTip(QString("<b>%1 (%2)</b><br/> Move the selected item(s) to the location you choose.").arg
                            (MOVE_TO->text(), MOVE_TO->shortcut().toString()));

        QAction* COPY_TO = new QAction(QIcon(":/themes/CP_TO_COMMAND_PATH"), "Copy to");
        COPY_TO->setShortcutVisibleInContextMenu(true);
        COPY_TO->setToolTip(QString("<b>%1 (%2)</b><br/> Copy the selected item(s) to the location you choose.").arg
                            (COPY_TO->text(), COPY_TO->shortcut().toString()));
        QActionGroup* actionGroup = new QActionGroup(this);
...
//        SELECT_ALL->setShortcut(QKeySequence(Qt::ControlModifier | Qt::Key_A));
        ...
        SELECT_NONE->setToolTip(QString("<b>%1 (%2)</b><br/> Clear all your selections.").arg
                                (SELECT_NONE->text(), SELECT_NONE->shortcut().toString()));
    ...
    auto GetCOPY_PATHActions() -> QActionGroup*{
        ...
        COPY_FULL_PATH->setToolTip(QString("<b>%1 (%2)</b><br/>. <b>path/name</b> <br/>Copy the absolute file name of the selected item(s) to the clipboard.").arg
                                   (COPY_FULL_PATH->text(), COPY_FULL_PATH->shortcut().toString()));
        COPY_FULL_PATH->setShortcutVisibleInContextMenu(true);

        QAction* COPY_PATH = new QAction(QIcon(":/themes/COPY_PATH"), "Copy path");
        COPY_PATH->setToolTip(QString("<b>%1 (%2)</b><br/> <b>path</b>/name <br/>Copy the directory of the selected item(s) to the clipboard.").arg
                              (COPY_PATH->text(), COPY_PATH->shortcut().toString()));
        COPY_PATH->setShortcutVisibleInContextMenu(true);

        QAction* COPY_NAME = new QAction(QIcon(":/themes/COPY_NAME"), "Copy name");
        COPY_NAME->setToolTip(QString("<b>%1 (%2)</b><br/> path/<b>name</b> <br/>Copy the name of the selected item(s) to the clipboard.").arg
                              (COPY_NAME->text(), COPY_NAME->shortcut().toString()));
        COPY_NAME->setShortcutVisibleInContextMenu(true);

        QAction* COPY_THE_PATH = new QAction(QIcon(":/themes/COPY_THE_PATH"), "Copy the path");
        COPY_THE_PATH->setToolTip(QString("<b>%1 (%2)</b><br/> <b>pth/itemName.jpg</b> <br/>Given current selected item named 'itemName' and its path 'pth', <br/>'pth/itemName.jpg' is copied to the clipboard.").arg
                                  (COPY_THE_PATH->text(), COPY_THE_PATH->shortcut().toString()));
    ...
    auto GetNEWActions() -> QActionGroup*{
        ...
        NEW_FOLDER->setToolTip(QString("<b>%1 (%2)</b><br/> Create a new folder in current view.").arg
                               (NEW_FOLDER->text(), NEW_FOLDER->shortcut().toString()));
        ...
        NEW_TEXT_FILE->setToolTip(QString("<b>%1 (%2)</b><br/> Create a new text document in current view.").arg
                                  (NEW_TEXT_FILE->text(), NEW_TEXT_FILE->shortcut().toString()));
        ...
        NEW_JSON_FILE->setToolTip(QString("<b>%1 (%2)</b><br/> Create a new json file in current view.").arg
                                  (NEW_JSON_FILE->text(), NEW_JSON_FILE->shortcut().toString()));
        ...
        connect(actionGroup, &QActionGroup::triggered, this, [](QAction* act)->void{
            PreferenceSettings().setValue(MemoryKey::DEFAULT_NEW_CHOICE.name, act->text());
        });
        return actionGroup;
    }
