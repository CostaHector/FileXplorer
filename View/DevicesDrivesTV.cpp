...
void DevicesDrivesTV::onUpdateVolumes() {
  ...
  if (ret != FD_OK) {
    LOG_ERR_P("Update Volume(s) FAILED", "errorCode:%d", ret);
    return;
  }
  LOG_OK_P("Update Volume(s) Ok", "Insert:%d, Delete:%d, Update:%d",
...
void DevicesDrivesTV::onMountADriver() {
  ...
  if (!MountHelper::MountVolume(guid, label, volMountPoint)) {
    LOG_ERR_P("Mount Volume(s) failed", "guid:%s", qPrintable(guid));
    return;
  }
  LOG_OK_P("Mount Volume(s) ok", "guid:%s label:%s, volMountPoint:%s", qPrintable(guid), qPrintable(label), qPrintable(volMountPoint));
...
void DevicesDrivesTV::onUnmountADriver() {
  ...
  if (!MountHelper::UnmountVolume(mountedPnt)) {
    LOG_ERR_P("Unmount Failed", "Unmount Volume(s)[%s] from pnt:%s", qPrintable(guid), qPrintable(mountedPnt));
    return;
  }
  LOG_OK_P("Unmount Ok", "Unmount Volume(s)[%s] from pnt:%s", qPrintable(guid), qPrintable(mountedPnt));
